      ...
      {usuario.map((usuarioData) => (
            ...
            >
              ...
              <Link className="btn btn-outline-secondary mx-2"
                ...
                >View</Link>
                <Link className="btn btn-outline-secondary mx-2" to={`/editusers/${usuarioData.id}`}>Editar</Link>
                <IconButton aria-label="delete" onClick={()=>deletarUsuario(usuarioData.id)}>
        <DeleteIcon />
      </IconButton> 
            </TableRow>
          ))}
